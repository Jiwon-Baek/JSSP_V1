...
N_OPERATION = 100
N_MACHINE = 10
1 4523 6 7 8 9 => 1 2 4 5 6 3 7 8 9 => 654789312
1 2345 7 8 9 6

1. 3~6 번째 순서로 등장하는 OP들을 crossover 하고싶어. ( 23 18 32 7)
2. Parents 2에서 3~6 번쨰에 등장하는 애들도 23 18 32 7 라는 보장이 없으
3. Parents 2에서 3~6에 등장하는 애들을 보니, 23 5 12 9임
4. 전체 5 9 7 23 18 32를 다 묶어서 하기로 함
5. Parents 1에서는 5 23 18 32 7 9 의 순서로 등장
Parents 2에서서는 23 5 12 9 7 18 32
6. Child 1, 2를 얻는다

1 1 1 2 2 2 3 3 3
temp = 0
temp < idx < temp + opnum(3) => 0 1 2
0 1 2 => 0으로 바꿔버리기

90 91 92 ... 99 -> 9
0 1 2 .. 9 9 9 9 9 9 9 9 9 9 -> 0
...
class Machine:
    def __init__(self, env, id):
        ...
        self.queue = simpy.Store(self.env) # Job2_Op3 1 0 순서대로 하렴
...
def crossover(p1, p2, n):
    """
    N point crossover
    The parameter N is available for 2 or 3 only
...
#endregion GA_selection

#region GA_Reproduction
def save_elite_than_reproduce(saved, parents):
    ...
    for i in range(200):
        ...
        ################################## 방법2. 2 point crossover
        c1, c2 = crossover(p1, p2, 3)

        ################################## 방법3. 50%의 확률로 2 point, 50%의 확률로 3 point crossover
...
def modify_population_parameters(num_popul, n_family):  # 500, 10
    """
    Return number of parents set and the required number of reproduction for desired outcome (given as input)
    """
...
1. Hyperparameter 결정
modify_population_parameters : 원하는 parents 쌍의 조합과 한 parents 당 낳아야 하는 자식 수를 자동으로 구해주는 함수
modify_population_parameters(500, 50) : 500의 population 중 50쌍을 활용하는 경우, parents는 100마리, 2명당 자식 10마리를 낳아야 한다는 것을 알려줌
직접 계산해서 변수 지정해도 상관없음 (다만 여러번 디버깅 하기 편리하게끔 함수화 해놓은 것)

2. population 생성 (POP : Individual 객체를 담은 리스트)
POP = initiailize_population() -> Global variable 값만큼 Individual을 생성

3. for i in range (Number of Termination) : selection 방법을 roulette wheel, elite1, elite2 중 하나를 골라서 실행
convergence라는 변수에 결과값이 저장될 예정
그리고 reproduce()를 통해 새로운 POP 리스트가 반환됨
...
# 파일 저장, 파일명은 디버깅하기 편하게 랜덤한 숫자로 할 예정 -> 기억해놨다가 나중에 Visualization 코드 실행해야 함
path = os.getcwd()
